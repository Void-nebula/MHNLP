...
def main():
    ...
    df = pd.read_csv('Datasets/HMS_2016-2017_PUBLIC.csv', encoding_errors='ignore', low_memory=False)
    
    num_rows, num_columns = df.shape

    print("original data number: ", num_rows)

    depression_columns = ['dx_dep_1', 'dx_dep_2', 'dx_dep_5', 'dx_dep_6', 'dx_dep_5_text']

    anxiety_columns = ['dx_ax_1', 'dx_ax_2', 'dx_ax_3', 'dx_ax_4', 'dx_ax_5', 'dx_ax_9', 'dx_ax_10', 'dx_ax_9_text']
    ...
    def classify_depression_state(row):
        ...
        if pd.notna(row['dx_dep_1']):
            ...
            return 2
        # No Premenstrual in 2016
        elif pd.notna(row['dx_dep_5']) or pd.notna(row['dx_dep_5_text']):
            return 4
        elif pd.notna(row['dx_dep_6']):
            return 5
        else:
            return 0
    ...
    def classify_anxious_state(row):
        if pd.notna(row['dx_ax_1']):
            ...
            return 5
        elif pd.notna(row['dx_ax_9']) or pd.notna(row['dx_ax_9_text']):
            return 6
        elif pd.notna(row['dx_ax_10']):
            return 7
        else:
            return 0
    ...
    df['text'] = df.apply(lambda row: (
        ...
        # Financial situation
        "Financial situation: " + 
        (f"{row['fincur']}" if (pd.notna(row['fincur']) and row.get('fincur') != '') else "Unknown") + '. ' +

        # Sense of belonging
        (f"{"I " + row['belong1']}" if (pd.notna(row['belong1']) and row.get('belong1') != '') else "I dont't know how I agree") + 
        " with that I see myself as a part of the campus community. " +

        # time spent on class
        "I usually spend " + 
        (f"{row['timeclass']}" if (pd.notna(row['timeclass']) and row.get('timeclass') != '') else "unknown hours") + " attending class/lab. " +

        # time spent on studying
        "I usually spend " + 
        (f"{row['timestud']}" if (pd.notna(row['timestud']) and row.get('timestud') != '') else "unknown hours") + " studying/doing homework. " +

        # Confidence in academic performance
        (f"{"I " + row['persist'].lower()}" if (pd.notna(row['persist']) and row.get('persist') != '') else "am not sure") +
        " that I am confident I will be able to finish my degree no matter what challenges I may face. " + 

        # Satisfaction with social life
        "I " + 
        (f"{row['satisfied_overall']}" if (pd.notna(row['satisfied_overall']) and row.get('satisfied_overall') != '') else "don't know if I safisfied") + 
