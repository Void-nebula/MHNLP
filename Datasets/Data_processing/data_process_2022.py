...
def main():
    ...
    df = pd.read_csv('Datasets/HMS_2022-2023_PUBLIC_instchars.csv', encoding_errors='ignore', low_memory=False)
    
    num_rows, num_columns = df.shape

    print("original data number: ", num_rows)

    depression_columns = ['dx_dep_1', 'dx_dep_2', 'dx_dep_3', 'dx_dep_4_text', 'dx_dep_4', 'dx_dep_5']

    anxiety_columns = ['dx_ax_1', 'dx_ax_2', 'dx_ax_3', 'dx_ax_4', 'dx_ax_5', 'dx_ax_6', 'dx_ax_6_text', 'dx_ax_7',]
    ...
    def classify_anxious_state(row):
        ...
        if pd.notna(row['dx_ax_1']):
            ...
            return 5
        elif pd.notna(row['dx_ax_6']) or pd.notna(row['dx_ax_6_text']):
            return 6
        elif pd.notna(row['dx_ax_7']):
            return 7
        else:
            return 0
